...
sed "s,\#\RUSTFLAGS=\"-Cforce-frame-pointers=yes\",RUSTFLAGS=\"-Cforce-frame-pointers=yes -C target-cpu=native -C link-arg=-fuse-ld=mold\",g" -i /mnt/etc/makepkg.conf
...
#Use mold linker
sed "s,-z\,now,-z\,now -fuse-ld=mold,g" -i /mnt/etc/makepkg.conf
