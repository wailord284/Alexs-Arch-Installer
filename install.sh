...
if [ "$boot" = bios ] || [ "$boot" = efi ]; then
	...
	if [ "$filesystem" = btrfs ] ; then
		...
		mount -o noatime,compress-force=zstd:3,space_cache=v2,nodiscard,autodefrag,commit=90 -U "$rootTargetDiskUUID" /mnt
		...
		mount -o noatime,compress-force=zstd:3,space_cache=v2,nodiscard,autodefrag,commit=90,subvol=@var_log -U "$rootTargetDiskUUID" /mnt/var/log
		mount -o noatime,compress-force=zstd:3,space_cache=v2,nodiscard,autodefrag,commit=90,subvol=@var_cache -U "$rootTargetDiskUUID" /mnt/var/cache
		mount -o noatime,compress-force=zstd:3,space_cache=v2,nodiscard,autodefrag,commit=90,subvol=@var_tmp -U "$rootTargetDiskUUID" /mnt/var/tmp
		mount -o noatime,compress-force=zstd:3,space_cache=v2,nodiscard,autodefrag,commit=90,subvol=@opt -U "$rootTargetDiskUUID" /mnt/opt
		mount -o noatime,compress-force=zstd:3,space_cache=v2,nodiscard,autodefrag,commit=90,subvol=@srv -U "$rootTargetDiskUUID" /mnt/srv
	elif [ "$filesystem" = f2fs ] ; then
...
if [ "$encrypt" = y ]; then
	sed "s,HOOKS=(systemd autodetect modconf kms keyboard keymap block filesystems fsck),HOOKS=(base udev keyboard autodetect modconf kms keymap block encrypt filesystems fsck),g" -i /mnt/etc/mkinitcpio.conf
fi
